...
class LSTM_CNN_CRF:
    ...
    def __init__(self, embedding="glove", settings='data.json'):
        ...
        print(" ======== " + embeddings[self.embed_flag] + " + LSTM - CNN - CRF ========")
        self.results_dir = 'lcc_' + embedding + "_results"
        self.dataset_dir = ''
        self.data_dir = ''
        ...
        handlers = [
            logging.FileHandler(str(Path(self.results_dir, 'lcc.log'))),
...
# ======================================================================================================================
    def model_fn(self, features, labels, mode):
        ...
        variable = tf.get_variable('chars_embeddings', [num_chars + 1, self.params['dim_chars']], tf.float32)
        char_embeddings = tf.nn.embedding_lookup(variable, char_ids)
        char_embeddings = tf.layers.dropout(char_embeddings, rate=dropout, training=training)

        # Char 1d convolution
        weights = tf.sequence_mask(nchars)
        char_embeddings = utilities.masked_conv1d_and_max(
    ...
    def generator_fn(words, tags):
        with Path(words).open('r', encoding="utf-8") as f_words, Path(tags).open('r', encoding="utf-8") as f_tags:
            for line_words, line_tags in zip(f_words, f_tags):
                yield LSTM_CNN_CRF.parse_fn(line_words, line_tags)
...
# ======================================================================================================================
    ...
    def input_fn(words, tags, params=None, shuffle_and_repeat=False):
        ...
        dataset = tf.data.Dataset.from_generator(
                        functools.partial(LSTM_CNN_CRF.generator_fn, words, tags),
