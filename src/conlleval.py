...
def get_result(correct_chunks, true_chunks, pred_chunks,
    ...
    res = ""
    res += "processed %i tokens with %i phrases; " % (sum_true_counts, sum_true_chunks)
    res += "found: %i phrases; correct: %i.\n" % (sum_pred_chunks, sum_correct_chunks)

    res += "accuracy: %6.2f%%; (non-O)\n" % (100 * nonO_correct_counts / nonO_true_counts)
    res += "accuracy: %6.2f%%; " % (100 * sum_correct_counts / sum_true_counts)
    res += "precision: %6.2f%%; recall: %6.2f%%; FB1: %6.2f" % (prec, rec, f1)
